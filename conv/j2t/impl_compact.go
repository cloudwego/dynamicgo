...
func writeCompactRequestBaseToThrift(ctx context.Context, buf *[]byte, field *thrift.FieldDescriptor) error {
	...
	if bobj != nil {
		if b, ok := bobj.(*base.Base); ok && b != nil {
			...
			*buf = (*buf)[:l+3]
			thrift.CompactProtocol{}.EncodeFieldBegin((*buf)[l:l+3], field.Type().Type(), field.ID())
			l = len(*buf)
			*buf = (*buf)[:l+n]
...
func (self *CompactConv) handleUnmatchedFields(ctx context.Context, fsm *types.J2TStateMachine, desc *thrift.StructDescriptor, buf *[]byte, pos int, req http.RequestGetter, top bool) (bool, error) {
	if req == nil {
...
func (self *CompactConv) handleValueMapping(ctx context.Context, fsm *types.J2TStateMachine, desc *thrift.StructDescriptor, buf *[]byte, pos int, src []byte) (bool, error) {
