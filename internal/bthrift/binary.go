...
func (binaryProtocol) WriteStringNocopy(buf []byte, binaryWriter BinaryWriter, value string) int {
	return Binary.WriteBinaryNocopy(buf, binaryWriter, rt.Str2Mem(value))
}
...
func (binaryProtocol) StringLengthNocopy(value string) int {
	return Binary.BinaryLengthNocopy(rt.Str2Mem(value))
}
...
func (binaryProtocol) ReadMessageBegin(buf []byte) (name string, typeID thrift.TMessageType, seqid int32, length int, err error) {
	...
	length += l
	if e != nil {
		err = NewProtocolError(e)
		return
	}
	if size > 0 {
		err = NewProtocolErrorWithType(BadVersion, "Missing version in ReadMessageBegin")
		return
	}
	typeID = thrift.TMessageType(size & 0x0ff)
	version := int64(size) & thrift.VERSION_MASK
	if version != thrift.VERSION_1 {
		err = NewProtocolErrorWithType(BadVersion, "Bad version in ReadMessageBegin")
		return
	}
...
func (binaryProtocol) ReadByte(buf []byte) (value int8, length int, err error) {
	if len(buf) < 1 {
		return value, length, NewProtocolErrorWithType(thrift.INVALID_DATA, "[ReadByte] buf length less than 1")
	}
...
func (binaryProtocol) ReadI16(buf []byte) (value int16, length int, err error) {
	if len(buf) < 2 {
		return value, length, NewProtocolErrorWithType(thrift.INVALID_DATA, "[ReadI16] buf length less than 2")
	}
...
func (binaryProtocol) ReadI32(buf []byte) (value int32, length int, err error) {
	if len(buf) < 4 {
		return value, length, NewProtocolErrorWithType(thrift.INVALID_DATA, "[ReadI32] buf length less than 4")
	}
...
func (binaryProtocol) ReadI64(buf []byte) (value int64, length int, err error) {
	if len(buf) < 8 {
		return value, length, NewProtocolErrorWithType(thrift.INVALID_DATA, "[ReadI64] buf length less than 8")
	}
...
func (binaryProtocol) ReadDouble(buf []byte) (value float64, length int, err error) {
	if len(buf) < 8 {
		return value, length, NewProtocolErrorWithType(thrift.INVALID_DATA, "[ReadDouble] buf length less than 8")
	}
...
func (binaryProtocol) ReadString(buf []byte) (value string, length int, err error) {
	...
	if size < 0 || int(size) > len(buf) {
		return value, length, NewProtocolErrorWithType(thrift.INVALID_DATA, "[ReadString] the string size greater than buf length")
	}
...
func (binaryProtocol) ReadBinary(buf []byte) (value []byte, length int, err error) {
	...
	if size < 0 || int(size) > len(buf) {
		return value, length, NewProtocolErrorWithType(thrift.INVALID_DATA, "[ReadBinary] the binary size greater than buf length")
	}
