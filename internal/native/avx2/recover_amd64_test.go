...
func TestRecover_tb_skip(t *testing.T) {
	var v = types.NewTStateMachine()
	var sp = []byte{0xb, 0x0, 0x1, 0x0, 0x0, 0x0, 0x1, 0x64, 0x0}
	t.Run("v", func(t *testing.T) {
		defer func() {
			if r := recover(); r!= nil {
				t.Log("recover: ", r)
	...
	t.Run("sp", func(t *testing.T) {
		...
		_ = tb_skip(v, nil, len(sp), 12)
	})
}

func TestRecover_j2t_fsm_exec(t *testing.T) {
	// var v = types.NewJ2TStateMachine()
	var sp = `{"a":"b"}`
