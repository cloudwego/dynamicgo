...
func TestCompactUnwrapReply(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			name: "simple",
			args: args{
				proto: meta.EncodingThriftCompact,
				msg: general.Message{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			// pair of buffer
			inputBytes, err := tiCompact.Marshal(tt.args.msg)
			if err != nil {
				t.Fatalf("failed to marshal input %s", err)
			}
			wantBytes, err := tiCompact.Marshal(tt.args.msg.Arguments[1].(general.Struct))
			if err != nil {
				t.Fatalf("failed to marshal want bytes %s", err)
			}
			//
			_, _, _, _, got, err := UnwrapCompactMessage(tt.args.proto, inputBytes)
			...
			header, footer, err := GetCompactMessageHeaderAndFooter(tt.args.msg.MessageHeader.MessageName, TMessageType(tt.args.msg.MessageHeader.MessageType), 1, 0)
			if err != nil {
