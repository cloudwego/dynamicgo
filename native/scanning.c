...
int64_t advance_dword(const GoString *src, long *p, long dec, int64_t ret, uint32_t val)
{
    ...
    {
        *p -= dec;
        for (int i = 0; src->buf[*p] == (val & 0xff); i++, ++*p)
        {
            val >>= 8;
        }
        return -ERR_INVAL;
...
#define m0_mask(add)            \
    ...
    while (likely(nb >= 64))
    {
        ...
        if (unlikely(m2 != 0))
        {
            ep_setx(sp - ss + __builtin_ctzll(m2)) return -ERR_INVAL;
        }
...
long value(const char *s, size_t n, long p, JsonState *ret, int allow_control)
{
    ...
    case '"':
        ...
        return q;
    case 'n':
        ret->vt = advance_dword(&m, &q, 1, V_NULL, VS_NULL);
        return q;
    case 't':
        ret->vt = advance_dword(&m, &q, 1, V_TRUE, VS_TRUE);
        return q;
    case 'f':
        ret->vt = advance_dword(&m, &q, 0, V_FALSE, VS_ALSE);
        return q;
    case '[':
        ret->vt = V_ARRAY;
        return q;
    case '{':
        ...
        return q;
    case ':':
        ret->vt = allow_control ? V_KEY_SEP : -ERR_INVAL;
        return allow_control ? q : q - 1;
    case ',':
        ret->vt = allow_control ? V_ELEM_SEP : -ERR_INVAL;
        return allow_control ? q : q - 1;
    case ']':
        ret->vt = allow_control ? V_ARRAY_END : -ERR_INVAL;
        return allow_control ? q : q - 1;
    case '}':
        ret->vt = allow_control ? V_OBJECT_END : -ERR_INVAL;
        return allow_control ? q : q - 1;
    case 0:
        ret->vt = V_EOF;
        return q;
...
void vnumber(const GoString *src, long *p, JsonState *ret)
{
    ...
    if (i < n && (s[i] == 'e' || s[i] == 'E'))
    {
        ...
        set_vt(V_DOUBLE)
            check_eof()
                parse_sign(esm)
                    check_digit() while (i < n && is_digit(s[i]))
        {
            if (exp < 10000)
            {
                exp = exp * 10 + (s[i] - '0');
...
static inline long fsm_exec(StateMachine *self, const GoString *src, long *p, int validate_flag)
{
    ...
    while (self->sp)
    {
        ...
        case '8': /* fallthrough */
        case '9':
            FSM_XERR(skip_positive(src, p));
            break;
        case '-':
            FSM_XERR(skip_negative(src, p));
            break;
        case 'n':
            FSM_XERR(advance_dword(src, p, 1, *p - 1, VS_NULL));
            break;
        case 't':
            FSM_XERR(advance_dword(src, p, 1, *p - 1, VS_TRUE));
            break;
        case 'f':
            FSM_XERR(advance_dword(src, p, 0, *p - 1, VS_ALSE));
            break;
        case '[':
            FSM_PUSH(self, FSM_ARR_0);
            break;
        case '{':
            FSM_PUSH(self, FSM_OBJ_0);
            break;
        case '"':
        {
            if (validate_flag == VALID_DEFAULT)
...
long skip_number(const char *sp, size_t nb)
{
    long di = -1;
    long ei = -1;
    long si = -1;
